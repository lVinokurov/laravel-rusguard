...
/**
 * This class stands for LoadTrassirConfiguration StructType
 * @subpackage Structs
 */
class LoadTrassirConfiguration extends AbstractStructBase
{
    /**
     * The deviceServerId
     ...
     * @var string|null
     */
    protected ?string $passwordSDK = null;
    /**
     * Constructor method for LoadTrassirConfiguration
     * @uses LoadTrassirConfiguration::setDeviceServerId()
     * @uses LoadTrassirConfiguration::setIp()
     * @uses LoadTrassirConfiguration::setPort()
     * @uses LoadTrassirConfiguration::setLogin()
     * @uses LoadTrassirConfiguration::setPassword()
     * @uses LoadTrassirConfiguration::setPasswordSDK()
     * @param string $deviceServerId
     * @param string $ip
     * @param int $port
     * @param string $login
     * @param string $password
     * @param string $passwordSDK
     */
    public function __construct(?string $deviceServerId = null, ?string $ip = null, ?int $port = null, ?string $login = null, ?string $password = null, ?string $passwordSDK = null)
    {
        $this
     ...
     * @param string $deviceServerId
     * @return \StructType\LoadTrassirConfiguration
     */
    ...
    /**
     * Get passwordSDK value
     * An additional test has been added (isset) before returning the property value as
     * this property may have been unset before, due to the fact that this property is
     * removable from the request (nillable=true+minOccurs=0)
     * @return string|null
     */
    public function getPasswordSDK(): ?string
    {
        return isset($this->passwordSDK) ? $this->passwordSDK : null;
    }
    /**
     * Set passwordSDK value
     * This property is removable from request (nillable=true+minOccurs=0), therefore
     * if the value assigned to this property is null, it is removed from this object
     * @param string $passwordSDK
     * @return \StructType\LoadTrassirConfiguration
     */
    public function setPasswordSDK(?string $passwordSDK = null): self
    {
        // validation for constraint: string
        if (!is_null($passwordSDK) && !is_string($passwordSDK)) {
            throw new InvalidArgumentException(sprintf('Invalid value %s, please provide a string, %s given', var_export($passwordSDK, true), gettype($passwordSDK)), __LINE__);
        }
        if (is_null($passwordSDK) || (is_array($passwordSDK) && empty($passwordSDK))) {
            unset($this->passwordSDK);
        } else {
